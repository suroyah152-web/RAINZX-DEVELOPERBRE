-- == RAINZX DEV | Sniper Arena :: REBUILD v3.0 ==
-- Rage + Legit Silent Aim + ESP + Chams + WalkSpeed + AutoShoot
-- Rebuilt from PuckAFK's rage/legit presets by RAINZX DEV. RAINZX DEV branding.
...
task.spawn(function()
	...
	local cfg = {
		...
		teamCheck = false,
		-- Rage / Aim mode:
		-- "Off" (no aim), "Legit" (hold-to-aim, normal FOV, headshot),
		-- "Rage" (always on, huge FOV, closest part, ignores walls/filters)
		mode = "Off",
		autoShoot = false,
		rageFov = 500,
	...
	local EntityService, Shootable do
		local okE, ent = pcall(function()
			return require(ReplicatedStorage:WaitForChild("Remote", 5):WaitForChild("EntityService", 5))
		end)
		local okS, sh = pcall(function()
			return require(ReplicatedStorage:WaitForChild("Client", 5)
				:WaitForChild("CombatController", 5)
				:WaitForChild("ClientComponent", 5)
				:WaitForChild("ClientShootableComponent", 5))
		end)
	...
	local function closestInFOV()
		...
		if not (isRage() or isLegit()) then return nil end
	...
	-- === Silent aim injection ===
	-- Wrapped in its own scope so a missing hookfunction/debug on the executor
	-- never stops the GUI or the other features from loading.
	local silentTarget = nil
	local function aimActive()
		if isRage() then return true end
		if isLegit() then return true end
		return false
	end
	do
		local ok = type(hookfunction) == "function" and type(debug) == "table" and debug.getupvalue
		if ok and Shootable and Shootable.LocalShoot then
			...
			if originFn then
				local old; old = hookfunction(originFn, function(...)
					...
					if aimActive() and silentTarget then
			...
			if targetFn then
				local old; old = hookfunction(targetFn, function(...)
					if aimActive() and silentTarget then return silentTarget.pos, silentTarget.part end
					return old(...)
				end)
			end
			local oldShoot; oldShoot = hookfunction(origShoot, function(self, ...)
				if aimActive() then silentTarget = closestInFOV() end
	...
	-- === ESP engine ===
	local espObjs = {} -- char -> {box=Quad, name=Text, health=Line}
	...
	local function renderESP(char, playerName)
		...
		e.health.Visible = cfg.espHealth and true or false
	...
	-- === Auto shoot (rage helper) ===
	-- Works with silent aim: when a target is inside the FOV the script
	-- fires a left click so the server registers the shot at the locked part.
	local autoShootCooldown = 0
	task.spawn(function()
		while task.wait(0.04) do
			if cfg.autoShoot and aimActive() and tick() >= autoShootCooldown then
				local t = closestInFOV()
				if t then
					pcall(function() mouse1click() end)
					autoShootCooldown = tick() + 0.1
				end
			end
		end
	end)
	...
	cycleButton(frame, 38, "Aim Mode", { "Off", "Legit", "Rage" },
		function() return cfg.mode end,
		function(v) cfg.mode = v end)
	toggleButton(frame, 72, "Auto Shoot", function() return cfg.autoShoot end, function(v) cfg.autoShoot = v end)
	toggleButton(frame, 106, "Headshot", function() return cfg.headshot end, function(v) cfg.headshot = v end)
	numeric(frame, 140, "FOV", "fov")
	toggleButton(frame, 174, "ESP", function() return cfg.esp end, function(v)
		cfg.esp = v
		if not v then hideAllESP() end
	end)
	toggleButton(frame, 208, "ESP Box", function() return cfg.espBox end, function(v) cfg.espBox = v end)
	toggleButton(frame, 242, "ESP Health", function() return cfg.espHealth end, function(v) cfg.espHealth = v end)
	toggleButton(frame, 276, "Chams", function() return cfg.chams end, function(v) cfg.chams = v end)
	numeric(frame, 310, "WalkSpeed", "speed")

	local close = Instance.new("TextButton")
	close.Size = UDim2.new(1, -20, 0, 26)
	close.Position = UDim2.new(0, 10, 0, 352)
	...
	print("RAINZX DEV | Sniper Arena loaded.")
end)
